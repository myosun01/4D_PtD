...
# ══════════════════════════════════════════════
# 12) 4D 워커 커널 파라미터 (v3.7)
# ══════════════════════════════════════════════
# [근거 없음 — 민감도 최우선 대상] 하루 중 작업 체류가 차지하는 비율.
# v3.1~v3.6 은 `horizon // 8` (=12.5%) 을 썼고 그 값에도 근거가 없었다.
# 0.75 는 2D 의 DWELL_STEPS/WORKDAY_STEPS (21,600/28,800) 와 같은 값을 가져온
# 것일 뿐, 실측이나 문헌값이 아니다. build/limitations.md 에 등록되어 있다.
DWELL_RATIO = 0.75

# [근거 없음] 출발 시각 분산을 하루 길이에 대한 비율로 환산한 값.
# 2D 는 START_STAGGER_S=120 초 / WORKDAY_STEPS=28,800 스텝 = 0.4167%.
# 4D 는 horizon 이 압축돼 있어 같은 비율을 적용하면 스텝 수가 0 이 될 수 있다.
# 0 이면 스태거가 없는 것이며, 그 사실을 로그에 남긴다(값을 올려 만들지 않는다).
STAGGER_RATIO = START_STAGGER_S / float(WORKDAY_STEPS)
