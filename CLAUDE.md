# CLAUDE.md — 4D PtD Simulation 프로젝트 규약
...
- 이 Windows 작업공간의 검증된 실행기는 `C:\Users\USER\anaconda3\python.exe`
  (Python 3.13)다. PATH의 `python`은 Python 3.14이며 프로젝트 의존성이 없다.
- 최초 1회 설치:
  `C:\Users\USER\anaconda3\python.exe -m pip install -r requirements.txt`
- 전체 빌드:
  `C:\Users\USER\anaconda3\python.exe scripts/build_all.py`
- 전체 테스트:
  `C:\Users\USER\anaconda3\python.exe -m pytest tests/ -q`
- `requirements.txt`는 v3.8에서 실제 검증한 패키지 버전을 고정한다.
