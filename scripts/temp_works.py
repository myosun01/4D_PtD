...
def main():
    ...
    #        + 3개 층 존치 (v3.6, KCS 14 20 12 3.3.2(2))
    #
    # [근거] KCS 14 20 12 3.3.2(2): "연속하여 시공하는 다층 구조의 경우 타설층을
    #   포함하여 최소 3개 층에 걸쳐 동바리를 존치하거나 적절하게 재설치한다."
    #   → 슬래브 N 의 동바리는 **N+2 슬래브 타설 후 해체**될 때까지 존치한다.
    #     (타설층 N+2 를 포함해 N, N+1, N+2 세 층에 동바리가 동시 존재)
    #
    # [설계 선택] zone 을 3배로 복제하지 않는다. 같은 동바리가 더 오래 서 있는
    #   것이므로 **zone 수는 그대로 두고 despawn 을 뒤로 미룬다.** 결과적으로
    #   여러 층의 동바리 zone 이 기간상 겹쳐 3개 층 동시 존재가 성립한다.
    #
    # [경계] 상부 2개 층이 없으면 있는 만큼만 적용한다(최상층 부근). 로그에 남긴다.
    #
    # [주의] 표 3.3-2(압축강도 미시험 시 재령 기준)는 「기초·보·기둥 및 벽의 측면」
    #   에만 적용되며 슬래브 밑면·동바리에는 쓸 수 없다. 따라서 존치기간을 **일수로
    #   환산하지 않고 층 단위로만** 모델링한다. (build/limitations.md 참조)
    all_slabs = []
    for _sy, _items in ifc["slabs"].items():
        for _it in _items:
            all_slabs.append(dict(_it, storey=_sy))

    # 타설 순서 = STOREY_ORDER 중 슬래브 타설 작업이 실제로 있는 층
    poured_order = [s for s in STOREY_ORDER if (tix.get(s) or {}).get("slab_pour")]

    def strip_after_two_levels(sy):
        """슬래브 sy 의 동바리 해체 시점 = sy 로부터 2개 층 위의 해체 작업.
        상부 층이 부족하면 있는 만큼(1개 위, 없으면 자기 층)을 쓴다.
        반환 (해체작업, 적용한 층수, 대상 층명)."""
        if sy not in poured_order:
            return (tix.get(sy) or {}).get("strip"), 0, sy
        i = poured_order.index(sy)
        for k in (2, 1, 0):
            if i + k < len(poured_order):
                tgt = poured_order[i + k]
                st = (tix.get(tgt) or {}).get("strip")
                if st is not None:
                    return st, k, tgt
        return (tix.get(sy) or {}).get("strip"), 0, sy
    ...
    for sy in STOREY_ORDER:
        ...
        # v3.6: 해체는 자기 층이 아니라 2개 층 위의 해체 작업에서 일어난다.
        strip_kcs, k_levels, tgt_storey = strip_after_two_levels(sy)
        extra["retention_rule"] = (
            "KCS 14 20 12 3.3.2(2) — 타설층 포함 최소 3개 층 존치. "
            "슬래브 %s 의 동바리는 %d개 층 위(%s)의 해체 작업에서 소멸한다."
            % (sy, k_levels, tgt_storey))
        extra["retention_levels_applied"] = k_levels + 1     # 자기 층 포함
        if k_levels < 2:
            extra["retention_boundary_note"] = (
                "상부 층이 부족해 3개 층을 채우지 못했다(적용 %d개 층). "
                "최상층 부근의 경계 처리다." % (k_levels + 1))
        r3_retention_log.append({
            "storey": sy, "shoring_level": below,
            "levels_applied": k_levels + 1,
            "despawn_storey": tgt_storey,
            "despawn_task": (strip_kcs or {}).get("task_id"),
            "prev_despawn_task": (strip or {}).get("task_id"),
        })
        B.add("H008_ShoringCollapse", below, proj, z_bot,
              max(0.1, z_top - z_bot),
              spawn_shore, trig(strip_kcs or strip), "zone_occupancy",
              [x["guid"] for x in ifc["slabs"][sy]], "SHORE", extra)
    n_r3 = len(B.zones) - n_r1 - n_r2

    log.append("")
    log.append("  R3 동바리 존치 (KCS 14 20 12 3.3.2(2) — 3개 층):")
    for x in r3_retention_log:
        log.append("    %-20s 동바리층 %-4s 적용 %d개 층 → 해체 %s (T-%s, 이전 T-%s)"
                   % (x["storey"], x["shoring_level"], x["levels_applied"],
                      x["despawn_storey"], x["despawn_task"], x["prev_despawn_task"]))
