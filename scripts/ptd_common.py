# -*- coding: utf-8 -*-
"""PtD v2.4 파이프라인 공용 모듈 — 44열 스키마, 경로, 어휘 매핑.

[v3.5] applies_to_cell_type / cell_type_basis 2열 추가 (42 → 44).
...
COLUMNS = [
    ...
    # 시뮬레이션 (8) — v3.5 에서 2열 추가
    "rule_type", "rule_id", "simulation_action", "parameter_value",
    "parameter_source", "sensitivity_target",
    # applies_to_cell_type: 규칙이 조작하는 격자 대상. v2.3 정본에서 이월되던 값을
    #   CSV 로 승격해 단일 원천으로 만든다. 비어 있으면 build_ttl 이 v2.3 이월값을 쓴다.
    # cell_type_basis: 그 값을 무엇에서 유도했는지(원천·규칙). 추측 배정 방지용 감사 열.
    "applies_to_cell_type", "cell_type_basis",
    # 관계 (4)
...
assert len(COLUMNS) == 44, len(COLUMNS)
