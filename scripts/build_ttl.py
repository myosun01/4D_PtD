...
def add_lifecycle_templates(g):
    ...
    [v3.6] CSV 가 선언한 template_id 는 **v2.3 이월값을 덮어쓴다**(CSV 가 정본).
    이전에는 이월된 것을 건너뛰어 CSV 로 기존 템플릿을 갱신할 수 없었다.
    덮어쓴 항목은 replaced 로 보고한다.
    """
    ...
    with io.open(SRC_TEMPLATES, encoding="utf-8-sig", newline="") as f:
        for r in csv.DictReader(f):
            ...
            s = P[tid]
            if tid in existing:
                g.remove((s, None, None))       # 이월값 제거 후 CSV 값으로 재선언
                replaced.append(tid)
...
# ── v3.6 신규 전거 선언 ────────────────────────────────────────────────
# lifecycle_templates.csv 의 references 열이 가리키는 Reference 중 v2.3 에 없는 것.
# 조문 원문은 build/limitations.md §KCS 에 전문을 실었다.
# evidenceLevel 은 v2.3 어휘를 따른다 (law / guideline / official_data ...).
# **원문 대조는 사람이 하지 않았으므로 comment 에 그 사실을 남긴다.**
NEW_REFERENCES = [
    ("REF_KCS_142012",
     "KCS 14 20 12 「거푸집 및 동바리」 — 3.3.1(1)(5), 3.3.2(2), 표 3.3-1, 표 3.3-2",
     "guideline",
     "국가건설기준(KCS). 3.3.2(2) 연속 시공 다층 구조에서 타설층 포함 최소 3개 층 "
     "동바리 존치. v3.6 에서 LCR_SHORING_COLLAPSE despawn 규칙의 근거로 도입. "
     "원문 대조 미완 — 조문 인용은 작업 지시서에 제시된 문구를 그대로 옮긴 것이다."),
]


def add_new_references(g):
    added = []
    for name, doc, level, note in NEW_REFERENCES:
        s = P[name]
        if (s, RDF.type, P.Reference) in g:
            continue
        g.add((s, RDF.type, P.Reference))
        g.add((s, P.sourceDocument, Literal(doc)))
        g.add((s, P.evidenceLevel, Literal(level)))
        g.add((s, RDFS.comment, Literal(note, lang="ko")))
        added.append(name)
    return added
...
def build():
    ...
    ref_added = add_new_references(g)
    if ref_added:
        print("   신규 전거 선언 (v3.6)          : %s" % ", ".join(ref_added))
    tpl_added, tpl_replaced = add_lifecycle_templates(g)
    if tpl_added or tpl_replaced:
        print("   LifecycleRuleTemplate 병합       : CSV 선언 %d (%s)"
              % (len(tpl_added), ", ".join(tpl_added) or "-"))
        if tpl_replaced:
            print("     └ v2.3 이월값을 CSV 로 갱신    : %s" % ", ".join(tpl_replaced))
    ...
    if ref_added:
        explained["Reference"] = (
            "기대 31 = v2.3 이월. 여기에 v3.6 신규 전거 %d건(%s)을 선언해 +%d → %d."
            % (len(ref_added), ", ".join(ref_added), len(ref_added),
               EXPECT["Reference"] + len(ref_added)))
    if mat:
    ...
    for k in ["KnowledgeEntry", "ExecutableAlternative", "Reference",
        ...
        delta = len(ref_added) if k == "Reference" else len(mat)
        if got == exp:
            verdict = "OK"
        elif k in explained and got == exp + delta:
            verdict = "차이 설명됨"
        else:
            verdict = "다름"
