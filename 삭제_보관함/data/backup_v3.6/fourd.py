...
def channel_cells(grid: np.ndarray, collapse_cells, drop_cells,
                  opening_buffer_cells=None) -> Dict[str, frozenset]:
    """그날 격자의 채널별 노출 셀 집합. collapse/drop은 바닥이 walkable이므로 별도 추적.

    [v3.5 A-2] fall = 개구부 인접 walkable 셀(2D 동일 정의) ∪ 개구부 주변 버퍼 셀.
    버퍼는 통행 가능한 바닥이며 노출이 발생하는 자리다 — 구멍이 아니다."""
    ...
    fall = set(fall_edge_cells(grid))                # 개구부 인접 walkable 셀
    if opening_buffer_cells:
        R, Co = grid.shape
        fall |= {(r, c) for (r, c) in opening_buffer_cells
                 if 0 <= r < R and 0 <= c < Co and walkable(r, c)}
    coll = frozenset((r, c) for (r, c) in collapse_cells if walkable(r, c))
    drop = frozenset((r, c) for (r, c) in drop_cells if walkable(r, c))
    return {"fall": frozenset(fall), "edge": edge, "material": material,
...
def apply_temp_structures(grid: np.ndarray, level_id: str, day: int,
    ...
    [v3.5 수정] walkable TS 는 **정적 격자의 FLOOR_OPENING 을 메우지 않는다.**
    TS1 데크는 슬래브 bbox 발자국 전체에서 파생되므로 개구부 위도 덮는데,
    실제 거푸집은 개구부 자리에 박스아웃을 두어 그 부분을 비운다. 메우면 영구
    개구부가 데크 존속 기간(타설 전 ~ 해체 완료) 내내 사라져 H001 노출이
    구조적으로 소멸한다. A-2 가 이 결함을 드러냈다."""
    if not temp_structures:
        return {}
    R, Co = grid.shape
    stat = {"walkable_added": 0, "blocked": 0, "opening_preserved": 0,
            "types": Counter()}
    for t in temp_structures.get(level_id, ()):
        ...
        if t.get("walkable"):
            for r, c in t["cells"]:
                if not (0 <= r < R and 0 <= c < Co):
                    continue
                if grid[r, c] == C.FLOOR_OPENING:
                    stat["opening_preserved"] += 1   # 박스아웃 — 메우지 않는다
                elif grid[r, c] == C.WALL:
...
def build_level_day(site: SiteModel, level_id: str, hazards, effects_by_instance: dict,
    ...
    collapse_cells, drop_cells = set(), set()
    opening_buffer_cells = set()                  # v3.5 A-2: 개구부 주변 위험대(통행 가능)
    cell_mult = {ch: {} for ch in CHANNELS}
    scaled_openings = []                          # (mult, [buffer cells]) — fall 배율용
    ...
    for h in hazards:
        ...
        if h.hazard_type == _HAZ_OPENING:
            # [v3.5 A-2] H001 zone 셀은 개구부 **본체가 아니라 주변 버퍼**다
            # (hazard_zones.json 의 폴리곤은 도넛이고 본체는 내부 링이며,
            #  본체 셀은 zone.cells 에 들어 있지 않다 — 검증: 본체 205셀이
            #  site.json 정적 격자에서 이미 100% FLOOR_OPENING).
            # 따라서 버퍼를 구멍으로 덮어쓰지 않는다. 통행 가능한 바닥으로 두고
            # fall 노출 대상에만 넣는다. 개구부 본체는 정적 격자가 이미 구멍이다.
            for (r, c) in h.cells:
                opening_buffer_cells.add((int(r), int(c)))
            if mult != 1.0:
                scaled_openings.append((mult, [(int(r), int(c)) for r, c in h.cells]))
        # [v3.5] 아래 오버레이들은 **개구부(FLOOR_OPENING)를 메우지 않는다.**
        # 단부·적재물 zone 이 개구부와 겹치면 구멍이 통행 가능한 셀로 덮여
        # 개구부가 사라진다(실측: H007 이 본체 셀 12개를 EDGE 로 덮고 있었다).
        # A-2 이전에는 H001 오버레이가 뒤이어 다시 구멍을 뚫어 가려져 있었다.
        elif h.hazard_type == _HAZ_EDGE:
            for (r, c) in h.cells:
                if grid[r, c] not in (C.WALL, C.FLOOR_OPENING):
                    grid[r, c] = C.EDGE
    ...
    ch = channel_cells(grid, collapse_cells, drop_cells, opening_buffer_cells)
    # H001 scale: 버퍼 셀 자체와 그 인접 fall 셀에 배율 부여
    for mult, bufset in scaled_openings:
        bset = set(bufset)
        for (r, c) in ch["fall"]:
            if (r, c) in bset or any((r + dr, c + dc) in bset
...
def instance_exposure_cells(h):
    """인스턴스 λ 귀속 셀. H001(fall)은 zone 셀 + 인접, 그 외는 인스턴스 셀.

    [v3.5 A-1] H001 은 zone 셀(=개구부 주변 버퍼)과 그 4방 인접을 **모두** 돌려준다.
    두 단계 모두에서 옳은 정의라 단계 간 대조가 가능하다:
      · A-2 이전 — 버퍼가 FLOOR_OPENING(구멍)이라 zone 셀에는 노출이 생길 수 없다.
        따라서 실질적으로 인접 링만 집계된다(기존 동작과 동일).
      · A-2 이후 — 버퍼가 walkable 이 되어 zone 셀에서 노출이 발생한다.
        같은 정의로 그것까지 집계된다.
    zone 셀만 보던 기존 집계는 fall 노출이 구조상 생길 수 없는 자리를 세고 있었다."""
    if h.hazard_type == _HAZ_OPENING:
        out = set()
        for (r, c) in h.cells:
            r, c = int(r), int(c)
